...
const Profile = () => {
  ...
  const { user, logout, isAuthenticated, updateProfile, updateProfilePic } = useAuth();
  ...
  const handleProfileImageChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      try {
        ...
        console.error('Failed to update profile picture. Please try again.', err);
      }
    }
  };
            ...
            <div className="flex flex-col md:flex-row md:items-center gap-8">
              <div className="relative">
                ...
                <label
                  ...
                  className="absolute bottom-0 right-0 bg-white p-2 rounded-full shadow-md cursor-pointer hover:bg-gray-100 transition-colors duration-200 dark:bg-gray-700 dark:hover:bg-gray-600 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {loading ? (
                    <div className="animate-spin">
                      <svg className="h-5 w-5 text-indigo-600 dark:text-indigo-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
